...
-------------------------------------------------------------------------
-- ADVANCED ANTI-RAGDOLL (BASED ON RZNNQ STYLE)
-------------------------------------------------------------------------
...
-- Palavras-chave de remotes
local RemoteKeywords = { "ragdoll", "combatservice", "useitem" }
...
local function ForceNormal(char)
    ...
    if not AntiRagdoll.Frozen then
        AntiRagdoll.Frozen = true
        root.Anchored = true
...
local function Release(char)
    local root = char:FindFirstChild("HumanoidRootPart")
    if root and AntiRagdoll.Frozen then
        root.Anchored = false
        AntiRagdoll.Frozen = false
    end
end
...
local function InitAntiItem()
    ...
    table.insert(AntiRagdoll.Connections,
...
local function InitCharacter(char)
    ...
    table.insert(AntiRagdoll.Connections,
        hum.StateChanged:Connect(function(_, new)
            if AntiRagdoll.Enabled and BlockedStates[new] then
                ForceNormal(char)
                RestoreMotors(char)
            end
        end)
    ...
    table.insert(AntiRagdoll.Connections,
        RunService.Stepped:Connect(function()
            if not AntiRagdoll.Enabled then
                Release(char)
                return
            end
...
function AntiRagdoll:Enable()
    ...
    if self.AntiItem then
        task.delay(0.3, InitAntiItem)
    end
end
...
function AntiRagdoll:IsEnabled()
    return self.Enabled
end
...
AddToggle("Combat", "Auto Meduse", false, function(v)
    toggleFloat(v)
end)

-------------------------------------------------------------------------
-- FLOAT MODULE (FIX REAL FUNCIONANDO + TOGGLE)
-------------------------------------------------------------------------
local FloatModule = {
    gui = nil,
    btn = nil,
...
-- FUNÇÃO DE ATIVAR FLOAT (SEPARADA PRA FUNCIONAR NO TOGGLE)
local function startFloat()
    ...
    FloatModule.floatForce = Instance.new("BodyVelocity")
    FloatModule.floatForce.MaxForce = Vector3.new(0, 50000, 0) -- 🔥 reduzido pra não kickar
    FloatModule.floatForce.P = 2000
    FloatModule.floatForce.Parent = hrp

    FloatModule.connection = RunService.Heartbeat:Connect(function()
        if not FloatModule.Active then return end
        if not hrp then return end

        local diff = (startY + 6) - hrp.Position.Y

        FloatModule.floatForce.Velocity = Vector3.new(0, diff * 2, 0)
    end)
end

local function stopFloat()
    if FloatModule.floatForce then
        FloatModule.floatForce:Destroy()
        FloatModule.floatForce = nil
    end

    if FloatModule.connection then
        FloatModule.connection:Disconnect()
        FloatModule.connection = nil
    end
end
...
local function createFloatUI()
    ...
    local ScreenGui = game.CoreGui:FindFirstChild("FloatUI") or Instance.new("ScreenGui")
    ScreenGui.Name = "FloatUI"
    ScreenGui.ResetOnSpawn = false
    ScreenGui.Parent = game.CoreGui

    FloatModule.gui = ScreenGui

    -- BOTÃO
    FloatModule.btn = Instance.new("TextButton")
    FloatModule.btn.Size = UDim2.new(0,120,0,40)
    FloatModule.btn.Position = UDim2.new(0.5,-60,0.7,0)
    FloatModule.btn.Text = "FLOAT"
    FloatModule.btn.Font = Enum.Font.GothamBold
    FloatModule.btn.TextSize = 14
    FloatModule.btn.TextColor3 = Color3.new(1,1,1)
    FloatModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
    FloatModule.btn.Parent = ScreenGui

    Instance.new("UICorner", FloatModule.btn)
    ...
    arrow.Parent = FloatModule.btn

    -- SAVE
    FloatModule.saveBtn = Instance.new("TextButton")
    FloatModule.saveBtn.Size = UDim2.new(0,120,0,28)
    FloatModule.saveBtn.Position = UDim2.new(0,0,1,4)
    FloatModule.saveBtn.Text = "Save"
    FloatModule.saveBtn.BackgroundColor3 = Color3.fromRGB(45,45,45)
    FloatModule.saveBtn.TextColor3 = Color3.new(1,1,1)
    FloatModule.saveBtn.Visible = false
    FloatModule.saveBtn.Parent = FloatModule.btn

    Instance.new("UICorner", FloatModule.saveBtn)

    -- abrir save
    arrow.MouseButton1Click:Connect(function()
        FloatModule.saveBtn.Visible = not FloatModule.saveBtn.Visible
    end)

    -- salvar posição
    FloatModule.saveBtn.MouseButton1Click:Connect(function()
        if writefile then
            local pos = {
                x = FloatModule.btn.Position.X.Offset,
                y = FloatModule.btn.Position.Y.Offset
            }
            writefile("float_pos.json", HttpService:JSONEncode(pos))
        end
    end)

    -- carregar posição
    if readfile and isfile and isfile("float_pos.json") then
        local data = HttpService:JSONDecode(readfile("float_pos.json"))
        FloatModule.btn.Position = UDim2.new(0,data.x,0,data.y)
    end

    -- DRAG
    local dragging, dragStart, startPos

    FloatModule.btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then

            dragging = true
            dragStart = input.Position
            startPos = FloatModule.btn.Position
    ...
    UIS.InputChanged:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            FloatModule.btn.Position = UDim2.new(
                startPos.X.Scale,
                startPos.X.Offset + delta.X,
                startPos.Y.Scale,
                startPos.Y.Offset + delta.Y
    ...
    -- CLICK
    FloatModule.btn.MouseButton1Click:Connect(function()

        FloatModule.Active = not FloatModule.Active

        if FloatModule.Active then
            FloatModule.btn.Text = "FLOAT ON"
            FloatModule.btn.BackgroundColor3 = Color3.fromRGB(0,170,0)
            startFloat()
        else
            FloatModule.btn.Text = "FLOAT"
            FloatModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
...
-- CRIAR UI
if not FloatModule.gui then
    createFloatUI()
end

-- TOGGLE (AGORA FUNCIONA DE VERDADE)
AddToggle("Combat", "FLOAT", false, function(v)

    FloatModule.gui.Enabled = v

    if v then
        -- ativa automaticamente
        FloatModule.Active = true
        FloatModule.btn.Text = "FLOAT ON"
        FloatModule.btn.BackgroundColor3 = Color3.fromRGB(0,170,0)
        startFloat()
    else
        FloatModule.Active = false
        stopFloat()
...
---------------------------------------------------------------------
-- AUTO PLAY (COMPLETO + FIX FINAL REAL)
---------------------------------------------------------------------
...
local function disconnectAll()
    for _,c in pairs(AutoPlay.Connections) do
        pcall(function() c:Disconnect() end)
    end
...
local function startAutoPlay()
    if AutoPlay.Thread then return end
    ...
    AutoPlay.Thread = task.spawn(function()
        ...
        local function go(pos,speed,cond)
            ...
            while cond() and AutoPlay.Enabled and (r.Position - pos).Magnitude > 1 do
        ...
        table.insert(AutoPlay.Connections,
...
if Saved and Saved.WalkSpeed then
    SPEED_STEAL = Saved.WalkSpeed
end
...
local function go(pos)
    ...
    while WalkModule.Active and (r.Position - pos).Magnitude > 1 do
        local dir = (pos - r.Position).Unit
...
-- DETECTA UMA VEZ E GUARDA
game:GetService("RunService").Heartbeat:Connect(function()
    if WalkModule.detectedMode then return end

    local r = hrp()
    if not r then return end

    if (r.Position - AUTO1_TRIGGER).Magnitude < 6 then
        WalkModule.detectedMode = 1
    elseif (r.Position - AUTO2_TRIGGER).Magnitude < 6 then
        WalkModule.detectedMode = 2
    end
end)

-- UI
local function createWalkUI()
    WalkModule.gui = Instance.new("ScreenGui")
    WalkModule.gui.Name = "WalkUI"
    WalkModule.gui.ResetOnSpawn = false
    WalkModule.gui.Enabled = false
    WalkModule.gui.Parent = CoreGui

    WalkModule.btn = Instance.new("TextButton")
    WalkModule.btn.Size = UDim2.new(0,120,0,40)

    if Saved and Saved.WalkPos then
        WalkModule.btn.Position = UDim2.new(
            Saved.WalkPos.XS,
            Saved.WalkPos.XO,
            Saved.WalkPos.YS,
            Saved.WalkPos.YO
        ...
        WalkModule.btn.Position = UDim2.new(0.5,-60,0.75,0)
    end

    WalkModule.btn.Text = "WALK"
    WalkModule.btn.Font = Enum.Font.GothamBold
    WalkModule.btn.TextSize = 14
    WalkModule.btn.TextColor3 = Color3.new(1,1,1)
    WalkModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
    WalkModule.btn.Parent = WalkModule.gui

    Instance.new("UICorner", WalkModule.btn).CornerRadius = UDim.new(0,8)
    ...
    arrowLeft.Font = Enum.Font.GothamBold
    arrowLeft.TextSize = 14
    arrowLeft.Parent = WalkModule.btn
    ...
    arrowRight.Font = Enum.Font.GothamBold
    arrowRight.TextSize = 14
    arrowRight.Parent = WalkModule.btn

    -- SAVE BTN
    WalkModule.saveBtn = Instance.new("TextButton")
    WalkModule.saveBtn.Size = UDim2.new(1,0,0,25)
    WalkModule.saveBtn.Position = UDim2.new(0,0,1,4)
    WalkModule.saveBtn.Text = "Save"
    WalkModule.saveBtn.Visible = false
    WalkModule.saveBtn.BackgroundColor3 = Color3.fromRGB(35,35,35)
    WalkModule.saveBtn.TextColor3 = Color3.new(1,1,1)
    WalkModule.saveBtn.Font = Enum.Font.Gotham
    WalkModule.saveBtn.TextSize = 13
    WalkModule.saveBtn.Parent = WalkModule.btn

    Instance.new("UICorner", WalkModule.saveBtn)

    arrowRight.MouseButton1Click:Connect(function()
        WalkModule.saveBtn.Visible = not WalkModule.saveBtn.Visible
    end)

    WalkModule.saveBtn.MouseButton1Click:Connect(function()
        if not Saved then Saved = {} end

        local pos = WalkModule.btn.Position
    ...
    -- CONFIG SPEED GUI
    WalkModule.speedGui = Instance.new("Frame")
    WalkModule.speedGui.Size = UDim2.new(0,200,0,60)
    WalkModule.speedGui.Position = WalkModule.btn.Position + UDim2.new(0,0,0,50)
    WalkModule.speedGui.BackgroundColor3 = Color3.fromRGB(20,25,30)
    WalkModule.speedGui.Visible = false
    WalkModule.speedGui.Parent = WalkModule.gui
    Instance.new("UICorner", WalkModule.speedGui)

    local box = Instance.new("TextBox")
    box.Size = UDim2.new(1,-20,0,25)
    box.Position = UDim2.new(0,10,0,15)
    box.Text = tostring(SPEED_STEAL)
    box.BackgroundColor3 = Color3.fromRGB(30,35,40)
    box.TextColor3 = Color3.new(1,1,1)
    box.Font = Enum.Font.GothamBold
    box.TextSize = 14
    box.Parent = WalkModule.speedGui
    Instance.new("UICorner", box)
    ...
    arrowLeft.MouseButton1Click:Connect(function()
        WalkModule.speedGui.Visible = not WalkModule.speedGui.Visible
    end)
    ...
    WalkModule.btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then

            dragging = true
            dragStart = input.Position
            startPos = WalkModule.btn.Position
    ...
    -- CLICK
    WalkModule.btn.MouseButton1Click:Connect(function()
        WalkModule.Active = not WalkModule.Active

        if WalkModule.Active then
            WalkModule.btn.BackgroundColor3 = Color3.fromRGB(255,120,120)

            task.spawn(function()
                if WalkModule.detectedMode == 1 then
                    go(A1)
                    go(A2)
                elseif WalkModule.detectedMode == 2 then
                    go(B1)
                    go(B2)
                end

                WalkModule.Active = false
                WalkModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
            end)
        else
            WalkModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
...
local function enableWalk()
    if WalkModule.Enabled then return end
    WalkModule.Enabled = true

    if not WalkModule.gui then
        createWalkUI()
    end

    WalkModule.gui.Enabled = true
end

-- DISABLE
local function disableWalk()
    WalkModule.Enabled = false
    WalkModule.Active = false

    if WalkModule.gui then
        WalkModule.gui.Enabled = false
    end
end

-- TOGGLE
AddToggle("Combat", "Walk", false, function(v)
    if v then
        enableWalk()
    else
        disableWalk()
...
---------------------------------------------------------------------
-- ANTI FPS DEVOURER (REMOVE ACESSÓRIOS)
---------------------------------------------------------------------
...
local function cleanCharacter(char)
    ...
    for _,obj in ipairs(char:GetChildren()) do
        -- Remove acessórios (chapéu, cabelo, etc)
        if obj:IsA("Accessory") then
            obj:Destroy()
        end

        -- Remove roupas pesadas
        if obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("ShirtGraphic") then
            obj:Destroy()
        end

        -- Remove coisas extras (partículas, etc)
...
local function enableAntiFPS()
    if AntiFPS.Enabled then return end
    AntiFPS.Enabled = true

    applyToAll()

    -- loop leve pra garantir
    AntiFPS.Conn = RunService.Heartbeat:Connect(function()
        for _,plr in ipairs(Players:GetPlayers()) do
            if plr.Character then
                cleanCharacter(plr.Character)
            end
...
local function disableAntiFPS()
    AntiFPS.Enabled = false

    if AntiFPS.Conn then
        AntiFPS.Conn:Disconnect()
        AntiFPS.Conn = nil
    end
end

-- TOGGLE (SEM BOTÃO NA TELA)
AddToggle("Visual", "Anti Fps DEVOURER", false, function(v)
    if v then
        enableAntiFPS()
    else
        disableAntiFPS()
...
---------------------------------------------------------------------
-- SPIN MODULE (HELI + MOBILE + SEM "SPIN ON")
---------------------------------------------------------------------
...
local function stopSpin()
    if SpinModule.spinObj then
        SpinModule.spinObj:Destroy()
        SpinModule.spinObj = nil
    end

    if SpinModule.att then
        SpinModule.att:Destroy()
        SpinModule.att = nil
    end
end
...
local function startSpin()
    ...
    SpinModule.spinObj = spin
    SpinModule.att = att
end

-- UI
local function createSpinUI()
    SpinModule.gui = Instance.new("ScreenGui")
    SpinModule.gui.Name = "SpinUI"
    SpinModule.gui.ResetOnSpawn = false
    SpinModule.gui.Enabled = false
    SpinModule.gui.Parent = game:GetService("CoreGui")

    -- BOTÃO
    SpinModule.btn = Instance.new("TextButton")
    SpinModule.btn.Size = UDim2.new(0,120,0,40)

    if Saved and Saved.SpinPos then
        SpinModule.btn.Position = UDim2.new(
            Saved.SpinPos.XS,
            Saved.SpinPos.XO,
            Saved.SpinPos.YS,
            Saved.SpinPos.YO
        ...
        SpinModule.btn.Position = UDim2.new(0.5,-60,0.6,0)
    end

    SpinModule.btn.Text = "SPIN"
    SpinModule.btn.Font = Enum.Font.GothamBold
    SpinModule.btn.TextSize = 14
    SpinModule.btn.TextColor3 = Color3.new(1,1,1)
    SpinModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
    SpinModule.btn.Parent = SpinModule.gui

    Instance.new("UICorner", SpinModule.btn)

    -- SETA ^
    SpinModule.arrow = Instance.new("TextButton")
    SpinModule.arrow.Size = UDim2.new(0,20,0,20)
    SpinModule.arrow.Position = UDim2.new(1,-22,0,-18)
    SpinModule.arrow.Text = "^"
    SpinModule.arrow.BackgroundTransparency = 1
    SpinModule.arrow.TextColor3 = Color3.new(1,1,1)
    SpinModule.arrow.Font = Enum.Font.GothamBold
    SpinModule.arrow.TextSize = 14
    SpinModule.arrow.Parent = SpinModule.btn

    -- SAVE
    SpinModule.saveBtn = Instance.new("TextButton")
    SpinModule.saveBtn.Size = UDim2.new(1,0,0,25)
    SpinModule.saveBtn.Position = UDim2.new(0,0,1,4)
    SpinModule.saveBtn.Text = "Save"
    SpinModule.saveBtn.Visible = false
    SpinModule.saveBtn.BackgroundColor3 = Color3.fromRGB(35,35,35)
    SpinModule.saveBtn.TextColor3 = Color3.new(1,1,1)
    SpinModule.saveBtn.Font = Enum.Font.Gotham
    SpinModule.saveBtn.TextSize = 13
    SpinModule.saveBtn.Parent = SpinModule.btn

    Instance.new("UICorner", SpinModule.saveBtn)
    ...
    SpinModule.btn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then

            dragging = true
            dragStart = input.Position
            startPos = SpinModule.btn.Position
    ...
    SpinModule.arrow.MouseButton1Click:Connect(function()
        saveOpen = not saveOpen
        SpinModule.saveBtn.Visible = saveOpen
    end)

    -- SALVAR POS
    SpinModule.saveBtn.MouseButton1Click:Connect(function()
        if not Saved then Saved = {} end

        local pos = SpinModule.btn.Position
    ...
    -- CLICK (SEM MUDAR TEXTO)
    SpinModule.btn.MouseButton1Click:Connect(function()
        SpinModule.Active = not SpinModule.Active

        if SpinModule.Active then
            SpinModule.btn.BackgroundColor3 = Color3.fromRGB(255,80,80)
            startSpin()
        else
            SpinModule.btn.BackgroundColor3 = Color3.fromRGB(43,160,235)
            stopSpin()
...
local function enableSpin()
    if SpinModule.Enabled then return end
    SpinModule.Enabled = true

    if not SpinModule.gui then
        createSpinUI()
    end

    SpinModule.gui.Enabled = true
end

-- DISABLE
local function disableSpin()
    SpinModule.Enabled = false
    SpinModule.Active = false

    if SpinModule.gui then
        SpinModule.gui.Enabled = false
    end
...
AddToggle("Combat","Spin Toggle",false,function(v)
    if v then
        enableSpin()
    else
        disableSpin()
...
---------------------------------------------------------------------
-- SPINBOT (HELI EXATO - SEM MUDAR NADA)
---------------------------------------------------------------------
...
local function startSpin()
    if SpinBot.spinObj then
        SpinBot.spinObj:Destroy()
        SpinBot.spinObj = nil
    end
    ...
    SpinBot.spinObj = Instance.new("AngularVelocity")
    SpinBot.spinObj.Attachment0 = att
    SpinBot.spinObj.AngularVelocity = Vector3.new(0,10,0) -- 🔥 IGUAL HELI
    SpinBot.spinObj.MaxTorque = math.huge
    SpinBot.spinObj.Parent = hrp
end
...
AddToggle("Combat","SpinBot",false,function(v)
    SpinBot.Enabled = v
