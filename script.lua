...
-- =============================================
if _G.HeroHubConnections then
    for _, conn in ipairs(_G.HeroHubConnections) do
        pcall(function() conn:Disconnect() end)
    end
end
_G.HeroHubConnections = {}
_G.HeroHubAlive = false -- signal old while loops to stop
task.wait() -- let old loops check the flag
_G.HeroHubAlive = true

local function trackConnection(conn)
    table.insert(_G.HeroHubConnections, conn)
    return conn
end
...
pcall(function()
    LocalPlayer.PlayerGui:FindFirstChild("HeroAdminPanel"):Destroy()
end)

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")
ScreenGui.ResetOnSpawn = false
ScreenGui.Name = "HeroAdminPanel"
ScreenGui.DisplayOrder = 999999
-- =============================================
-- ZENO TOGGLE BUTTON (ON UI - TOP RIGHT)
-- =============================================
local HeroToggleBtn = Instance.new("TextButton")
HeroToggleBtn.Name = "HeroToggle"
HeroToggleBtn.Size = isMobile and UDim2.new(0, 80, 0, 32) or UDim2.new(0, 90, 0, 32)
HeroToggleBtn.Position = UDim2.new(1, isMobile and -88 or -98, 0, 8)
HeroToggleBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
HeroToggleBtn.BackgroundTransparency = 0.28  -- black with ~72% opacity = transparency
HeroToggleBtn.Text = "Hero ▼"
HeroToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
HeroToggleBtn.Font = Enum.Font.GothamBold
HeroToggleBtn.TextSize = isMobile and 13 or 15
HeroToggleBtn.BorderSizePixel = 0
HeroToggleBtn.ZIndex = 1001
HeroToggleBtn.Parent = ScreenGui  -- keeps it anchored to main frame

local HeroCorner = Instance.new("UICorner")
HeroCorner.CornerRadius = UDim.new(1, 0)  -- 
HeroCorner.Parent = HeroToggleBtn

local HeroStroke = Instance.new("UIStroke")
HeroStroke.Color = Color3.fromRGB(60, 60, 60)  -- grey border
HeroStroke.Thickness = 2
HeroStroke.Parent = HeroToggleBtn

-- Hover
HeroToggleBtn.MouseEnter:Connect(function()
    TweenService:Create(HeroToggleBtn, TweenInfo.new(0.18), {
        BackgroundTransparency = 0.15,
    }):Play()
end)
HeroToggleBtn.MouseLeave:Connect(function()
    TweenService:Create(HeroToggleBtn, TweenInfo.new(0.18), {
...
-- Toggle
HeroToggleBtn.MouseButton1Click:Connect(function()
    UI_OPEN = not UI_OPEN
    if UI_OPEN then
        ...
        HeroToggleBtn.Text = "Zeno ▲"
    end
end)

-- Mobile touch
HeroToggleBtn.InputBegan:Connect(function(input)
...
ToggleUI.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
...
local function CreatePlayerRow(plr)
	...
	Semi.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
task.spawn(function()
	while _G.HeroHubAlive and ScreenGui.Parent do
		task.wait(30)
...
ToggleHandlers.auto_grabber = function(state)
    ...
    if state then
        task.spawn(function()
            while autoGrabberEnabled and _G.HeroHubAlive do
                local char = LocalPlayer.Character
                local humanoid = char and char:FindFirstChildOfClass("Humanoid")
...
local COLORS = {
    ...
    AccentBright = Color3.fromRGB(90, 90, 90),
    ...
    BindText = Color3.fromRGB(200, 200, 200),
    BindBG = Color3.fromRGB(20, 20, 20),
    MiniPanel = Color3.fromRGB(8, 8, 8),
    MiniTitle = Color3.fromRGB(12, 12, 12),
    MiniTitleBorder = Color3.fromRGB(40, 40, 40),
    ButtonBG = Color3.fromRGB(18, 18, 18),
    ButtonBorder = Color3.fromRGB(45, 45, 45),
...
do
    ...
    task.spawn(function()
        while _G.HeroHubAlive and task.wait(0.3) do
            pcall(function()
...
task.spawn(function()
    while _G.HeroHubAlive and task.wait(0.1) do
        pcall(function()
            local player = Players.LocalPlayer
...
do
    ...
    createMiniToggle(content, "Admin Spammer", 2, function(state)
        local player = game.Players.LocalPlayer
        local playerGui = player:WaitForChild("PlayerGui")
        local gui = playerGui:FindFirstChild("HeroAdminPanel")
...
task.spawn(function()
    local visible = true
    while _G.HeroHubAlive and ScreenGui and ScreenGui.Parent do
        task.wait(0.5)
        if ListeningId then
