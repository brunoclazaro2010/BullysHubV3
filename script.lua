...
do
    ...
    -- Store refreshBooster for startup
    Connections._refreshBooster = refreshBooster
end
...
do
    ...
    -- Store functions for startup
    Connections._startPlayerESP = startPlayerESP
    Connections._startFriendESP = startFriendESP
    Connections._enableXray = enableXray
end
...
task.spawn(function()
    ...
    openServerPanel()
    openBaseProtPanel()
    if Connections._startPlayerESP then Connections._startPlayerESP() end
    if Connections._startFriendESP then Connections._startFriendESP() end
    if Connections._enableXray then Connections._enableXray() end
    if Connections._refreshBooster then Connections._refreshBooster() end
    workspace.CurrentCamera.FieldOfView = 120
    task.wait(1)
    pcall(detectBaseSide)
end)
